...
\title{\textbf{Computational Validation of Adaptive Interface Control in TMI Quantum Systems}\\\large Расширенный отчёт v4 по вычислительным тестам 1--27}
\author{Salkutsan Aleksey}\date{7 мая 2026}\begin{document}\maketitle
\begin{abstract}
Этот отчёт обновляет вычислительную линию ТМИ до Tests 1--27. Версия v4 добавляет внешнюю симуляторную ветку Tests 23--27: raw Qiskit-Aer failure, Qiskit noise calibration, calibrated Qiskit-Aer validation и circuit-inverse correctness audit. Главная проверяемая цепочка остаётся узкой:
\[I_Q^{coh-auto}\Rightarrow T_2^{eff}\uparrow\Rightarrow P_{success}\uparrow.\]
Новый внешний результат:
\[Calibrate(I_{sim})\land Correct(I_{circuit})\Rightarrow P_{TMI}>P_{standard}>P_{base}.\]
Это не лабораторное доказательство физического поля; это воспроизводимая вычислительная и Qiskit-Aer симуляторная валидация прикладного механизма автоинтерфейсного управления.
\end{abstract}\tableofcontents\newpage
\section{Статус отчёта}
\[\boxed{\text{TMI Quantum Computational Validation v0.4}}\]
Главная научная особенность v4: внешний Qiskit-Aer перенос сначала провалился, затем был откалиброван, а затем был очищен от ошибки обратной схемы.
...
Test 23 created a simulator-independent standard-simulator protocol. The key update in v4 is the real Qiskit-Aer branch:
\[
...
\begin{figure}[H]\centering\includegraphics[width=0.86\linewidth]{figures/fig_v4_qiskit_progression_mean_p.png}\caption{Mean success probabilities in Tests 24, 26 and 27.}\end{figure}
\begin{figure}[H]\centering\includegraphics[width=0.86\linewidth]{figures/fig_v4_qiskit_delta_progression.png}\caption{$P_{TMI}-P_{standard}$: negative in Test 24, positive after calibration/correction.}\end{figure}
\begin{figure}[H]\centering\includegraphics[width=0.80\linewidth]{figures/fig_v4_qiskit_cells_passed.png}\caption{Passed circuit-channel cells: 0/16, 12/16, 16/16.}\end{figure}
...
\appendix\section{Состав пакета}
Пакет содержит PDF и LaTeX исходники на русском и английском, JSON/CSV summary, графики внешней Qiskit-Aer ветки, пакеты Tests 12--27 и статусные файлы Tests 24--27.
\end{document}
